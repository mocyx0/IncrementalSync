...
public class DataStorageTwoLevel implements DataStorage {
    ...
    private static final int FIRST_LEVEL_COUNT = 8000000 / 5;
        ...
        if (seq == -1) {
            if (level1.flag[level1Index] == FLAG_VALID) {
                ...
                for (int i = 0; i < CELL_COUNT; i++) {
                    logRecord.colData[i] = level1.colData[FIRST_LEVEL_COUNT * i + level1Index];
                }
                return logRecord;
            }
        } else {
    ...
    */
    ...
    void clearColData(int index) {
        ...
        for (int i = 0; i < CELL_COUNT; i++) {
            ...
            level1.colData[index + i * FIRST_LEVEL_COUNT] = 0;
        }
        //
    }
    ...
    public void doLog(LogBlock logBlock, byte[] data, int logPos) throws Exception {
        ...
        if (id <= Config.ALI_ID_MIN || preId <= Config.ALI_ID_MIN) {
            return;
        }
        */
